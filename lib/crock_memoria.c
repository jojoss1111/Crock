...
void *memoria_malloc(size_t tam_usuario) {
    ...
    crock_lock();
    void *r = malloc_interno(tam_usuario);
    crock_unlock();
    return r;
}
...
int memoria_checar(void *ptr) {
    ...
    int r = checar_bloco_interno(bloco);
    crock_unlock();
    ...
    for (struct arena *a = arena_primeira; a != NULL; a = a->next) {
        ...
        while (b != NULL) {
            if (b->magia == crock_canario(b, CROCK_BLOCO_MAGIA_OK)) {
                if (checar_bloco_interno(b) <= 0) corrompidos++;
            }
            b = b->next;
        }
    }
...
static void free_interno(void *ptr) {
    ...
    if (bloco->magia == crock_canario(bloco, CROCK_BLOCO_MAGIA_LIVRE)) {
        crock_falha(CROCK_ERRO_USE_AFTER_FREE);
        return;
    }
    if (bloco->magia != crock_canario(bloco, CROCK_BLOCO_MAGIA_OK)) {
        crock_falha(CROCK_ERRO_USE_AFTER_FREE);
        return;
    }
    if (!redzone_checar(bloco)) {
        crock_falha(CROCK_ERRO_MEMORIA);
        return;
    }
...
static void *realloc_interno(void *ptr, size_t novo_tam) {
    ...
    if (!redzone_checar(bloco)) {
        return crock_falha_ptr(CROCK_ERRO_MEMORIA);
    }
...
#ifdef CROCK_RASTREAR_ALOCACOES
        ptr_para_bloco(novo)->rastreio_arquivo = bloco->rastreio_arquivo;
        ptr_para_bloco(novo)->rastreio_linha   = bloco->rastreio_linha;
#endif
        direto_remover(bloco);
...
void *memoria_realloc(void *ptr, size_t novo_tam) {
    crock_lock();
    void *r = realloc_interno(ptr, novo_tam);
    crock_unlock();
    return r;
}
